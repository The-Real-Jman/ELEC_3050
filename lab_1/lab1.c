...
int main(void) {
 ...
 led1 = 0; //Initial LED1 state
 led2 = 1; //Initial LED2 state
 toggles = 0; //#times LED state changed

 /* Endless loop */
 while (1) { //Can also use: for(;;) {
	if (led1 == 0){ //LED off?
		GPIOC->BSRR = 0x0100 << 16; //Reset PC8=0 to turn OFF blue LED (in BSRR 
			// upper half)
		GPIOC->BSRR = 0x0200; //Set PC9 to turn ON second LED
	}
	else{ //LED on
		GPIOC->BSRR = 0x0100; //Set PC8=1 to turn ON blue LED (in BSRR low
			// half)
		GPIOC->BSRR = 0x0200 << 16; //Reset PC9=9 to turn OFF second LED
	}
